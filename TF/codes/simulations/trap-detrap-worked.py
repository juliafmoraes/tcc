...
class TrapDetrap(Simulation):
    def __init__(self, dt, dx, T, n):
        ...
        self.K = 4 * math.pi * self.capture_radius
...
def run():
    ...
    for j in range(simulation.iterations + 1):
        ...
        for i in range(0, simulation.nodes - 1):
            gamma = ((simulation.k_t * N_dif[-1][i] * (simulation.trap_concentration - N_trap[-1][i])) -
                     (simulation.k_d * N_trap[-1][i])
...
#for slices
    ...
    writer = pd.ExcelWriter(r"C:\Users\Julia\Documents\tcc\TF\codes\results\20191012\slices\3\trap_detrap_9.xlsx")
